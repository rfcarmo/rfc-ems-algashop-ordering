...
public class ShoppingCartPersistenceEntityDisassembler {
    ...
    private Set<ShoppingCartItem> toItemsDomainEntities(Set<ShoppingCartItemPersistenceEntity> source) {
        return source.stream().map(this::toDomainItemEntity).collect(Collectors.toSet());
    }

    private ShoppingCartItem toDomainItemEntity(ShoppingCartItemPersistenceEntity source) {
